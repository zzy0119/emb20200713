...
int create_fsm(int fd1, int fd2)
{
	...
	evt.events = 0;
	evt.data.fd = fd1;
	if (epoll_ctl(epfd, EPOLL_CTL_ADD, fd1, &evt) == -1) {
		perror("epoll_ctl()");
		goto ERROR;
	}
	evt.events = 0;
	evt.data.fd = fd2;
	if (epoll_ctl(epfd, EPOLL_CTL_ADD, fd2, &evt) == -1) {
		perror("epoll_ctl()");
		goto ERROR;
	}
	...
	while (fsm12.state != STATE_T && fsm21.state != STATE_T) {
		...
		if ((revt.events & EPOLLIN && revt.data.fd == fd1) || (revt.events & EPOLLOUT && revt.data.fd == fd2))
			fsm_drive(&fsm12); // r tty1  w tty2
		if ((revt.events & EPOLLIN && revt.data.fd == fd2) || (revt.events & EPOLLOUT && revt.data.fd == fd1))
