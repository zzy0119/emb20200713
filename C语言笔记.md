...
2. 链表(链式存储)
   ...
   5. 动态库和静态库
      - 静态库:编译的时候加载,体积大
        - gcc -c a.c -o a.o
        - ar cr a.o libxxx.a
      - 动态库:编译阶段不加载，加载仅仅是函数接口的引用，调用时才加载，体积小
        - gcc -fPIC -shared -o libxxxx.so a.c b.c
        - gcc -o a.out main.c -lxxxx -Lpath
        - 将动态库放到库的默认路径/usr/lib 或/lib （不推荐）
        - 或者将动态库所在路径写入配置文件/etc/ld.so.conf 
        - sudo ldconfig 重新加载配置
        - ldd a.out查看此可执行文件所加载的所有动态库及路径
3. 栈
   - 特殊的线性结构，只允许再表的一端进行插入删除。插入过程称为入栈，删除过程称为出栈，允许插入删除的一端称为栈顶，另一端称为栈底
   - LIFO
   - 基本操作，创建栈，入栈，出栈，栈空，栈满, 销毁
4. 队列
   - 特殊的线性结构，只允许在表的一端插入，另一端删除。插入称为入队，删除称为出队。插入的那一端看成队尾，删除的一端为队头。
   - FIFO
   - 基本操作：创建，队空，队满，入队，出队，销毁
   - 链队 循环队列
